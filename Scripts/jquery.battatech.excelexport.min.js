(function ($) {
    $datatype = { Table: 1, Json: 2, Xml: 3, JqGrid: 4 }; var $defaults = { containerid: null, datatype: $datatype.Table, dataset: null, columns: null, returnUri: false }; var $settings = $defaults; $.fn.btechco_excelexport = function (options) {
        $settings = $.extend({}, $defaults, options); var gridData = []; var excelData; return Initialize(); function Initialize() {
            BuildDataStructure(); switch ($settings.datatype) {
                case 1: excelData = Export(ConvertFromTable()); break; case 2: excelData = Export(ConvertDataStructureToTable()); break; case 3: excelData =
                Export(ConvertDataStructureToTable()); break; case 4: excelData = Export(ConvertDataStructureToTable()); break
            } if ($settings.returnUri) return excelData; else window.open(excelData)
        } function BuildDataStructure() {
            switch ($settings.datatype) {
                case 1: break; case 2: gridData = $settings.dataset; break; case 3: $($settings.dataset).find("row").each(function (key, value) { var item = {}; if (this.attributes != null && this.attributes.length > 0) { $(this.attributes).each(function () { item[this.name] = this.value }); gridData.push(item) } }); break;
                case 4: $($settings.dataset).find("rows > row").each(function (key, value) { var item = {}; if (this.children != null && this.children.length > 0) { $(this.children).each(function () { item[this.tagName] = $(this).text() }); gridData.push(item) } }); break
            }
            ...
            excelFile += "</x:WorksheetOptions>"; excelFile += "</x:ExcelWorksheet>"; excelFile += "</x:ExcelWorksheets>"; excelFile += "</x:ExcelWorkbook>"; excelFile += "</xml>"; excelFile += "<![endif]--\x3e"; excelFile += "</head>"; excelFile += "<body>"; excelFile += htmltable.replace(/"/g, "'"); excelFile += "</body>"; excelFile += "</html>"; var uri = "data:application/vnd.ms-excel;base64,"; var ctx = { worksheet: name || "Worksheet", table: htmltable }; return uri + base64(format(excelFile, ctx))
