...
<script runat="server">
    ...
    private string TOKEN { get { return ViewState["TOKEN"] + ""; } set { ViewState["TOKEN"] = value; } }
            ...
            try
            {
                ...
                if (pageAccess != null)
                {
                    if (pageAccess.IsView == 1)
                    {
                        ...
                        applicationId = qu[0].ToString();
                        applicationType = qu[1].ToString();
                        token = qu[2].ToString();
                        TOKEN = token;

                        if (applicationId != "")
                        {
                            url = AppConfiguration.GetCamlifeAPIURL() + "Certificate/GetApplicationForm?applicationId=" + applicationId + "&applicationType=" + applicationType;
                            ...
                            reader.Close();
                            response.Close();
                            AppWebService.ResponeApplicationForm objCert = Newtonsoft.Json.JsonConvert.DeserializeObject<AppWebService.ResponeApplicationForm>(responseFromServer);
                            if (objCert.ApplicationForm != null)
                            {
                                Context.Response.Buffer = true;
                                Context.Response.Clear();
                                Context.Response.ContentType = "application/pdf";
                                //Context.Response.AddHeader("content-disposition", "attachment; filename=APP" + DateTime.Now.ToString("YYMMddhhmmss") + ".pdf");
                                Context.Response.BinaryWrite(objCert.ApplicationForm);
                    ...
                    {
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowErrorRedirect", "ShowErrorRedirect('You do not have permission to view application form.','Close','../../default.aspx');", true);
                    }
                }
                else
                {
    ...
    private void PrintApplications()
    {
        try
        {
            ...
            var appObj = Request.Cookies["CK_APP_NO_TEMP"].Value;
            //var obj = Newtonsoft.Json.JsonConvert.DeserializeObject<ApplicationCookie>(a);

            var qu = Request.QueryString;
            string url = AppConfiguration.GetCamlifeAPIURL() + "Certificate/GetApplicationForms";
